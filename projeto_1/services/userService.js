...
class userService{
    ...
    async loginUser(login, senha){
        try{
            ...
            if (senhaMatch) {
                const token = jwt.sign({ id: user.id }, secretKey, { expiresIn: '5h' }); // Ajuste a duração conforme necessário
    ...
    //findAllUser
    async findAllUser(page = 1, limit = 10) { // valores padrões para paginação
